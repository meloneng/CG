...
float scale = 0.47; // fator de escala proporcional em todos os eixos do modelo 3D
float rotx = 0.0, roty = 0.0, rotz = 0.0; // angulo de graus do modelo 3D no eixo x, y e z
float posx = 2.0, posy = 0.0, posz = -4.0; // deslocamento do modelo 3D no eixo x, y e z
...
void init_glut(const char *nome_janela, int argc, char** argv){
    ...
    //animation = glmLoadAnimation("models/frog/frogey.obj", paused, 1);
...
void display(void){
    ...
    glPushMatrix();
        ...
        glmDrawAnimation(animation, keyframe, mode);
    glPopMatrix();
    ...
    draw_text_stroke(x , 1.25, "(ESC)EXIT (</>)FPS: " + to_string(fps) + " FACE: " + to_string(faces) + " VERT: " + to_string(vertices) + " (P)PAUSE ANIM: ");
